...
fn update_bodies(bodies: &mut [Body], dt: f64) {
    ...
    let cloned_bodies = bodies.to_vec(); // Clone bodies before the closure

    cloned_bodies.par_iter().enumerate().for_each(|(i, body1)| {
        let mut acc = acc.lock().unwrap();
        for (j, body2) in cloned_bodies.iter().enumerate().skip(i + 1) { // Use cloned_bodies here
...
fn simulate_and_save(_seed: u64, steps: usize, filename: &str) -> io::Result<()> {
